...
- Review agent definitions before granting tool access — the production org is seeded from `packages/server/src/services/org-seed.ts`; `packages/engine/agents.yml` is the engine fallback set.
...
- `packages/engine/agents.yml` (engine fallback agents)
