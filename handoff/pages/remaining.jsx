...
// ====================== WORKSPACES ======================
// Redesigned around "open in place": no navigation away from this page.
// V1 — Master/detail split. List on left, live workspace pane on right.
// V2 — Inline expand. Click a row, it expands to show preview + tools below.
// V3 — Tabbed dock. Open multiple sandboxes as tabs above the table.

// ---- shared bits for the inline workspace pane ----
const WsPreviewMock = ({ port, theme = 'light' }) => (
  <div style={{ background: theme === 'dark' ? '#0b0d10' : '#fff', border: '1px solid var(--line)', borderRadius: 6, height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
    <div style={{ padding: '6px 10px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 6, background: 'var(--bg-1)' }}>
      <span style={{ width: 8, height: 8, borderRadius: 8, background: '#ff5f57' }}/>
      <span style={{ width: 8, height: 8, borderRadius: 8, background: '#febc2e' }}/>
      <span style={{ width: 8, height: 8, borderRadius: 8, background: '#28c840' }}/>
      <span className="mono" style={{ fontSize: 10, color: 'var(--ink-3)', marginLeft: 8, flex: 1 }} className="truncate">localhost:{port}/admin/dashboard</span>
      <Icon name="refresh" size={10} color="var(--ink-3)"/>
    </div>
    <div style={{ flex: 1, padding: 14, display: 'flex', flexDirection: 'column', gap: 8, fontSize: 10, color: theme === 'dark' ? '#9aa4b2' : 'var(--ink-3)' }}>
      <div style={{ height: 28, background: theme === 'dark' ? '#161a1f' : 'var(--bg-1)', borderRadius: 4, display: 'flex', alignItems: 'center', padding: '0 10px', fontWeight: 600, color: theme === 'dark' ? '#e6e8eb' : 'var(--ink)' }}>es-data-pipeline · admin</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6 }}>
        {[0,1,2].map(i => <div key={i} style={{ height: 44, background: theme === 'dark' ? '#13171b' : 'var(--bg-1)', borderRadius: 4, padding: 6 }}><div style={{ height: 8, width: '60%', background: theme === 'dark' ? '#222831' : 'var(--line)', borderRadius: 2, marginBottom: 4 }}/><div style={{ height: 14, width: '40%', background: theme === 'dark' ? '#2a313a' : 'var(--ink-2)', borderRadius: 2, opacity: 0.5 }}/></div>)}
      </div>
      <div style={{ flex: 1, background: theme === 'dark' ? '#13171b' : 'var(--bg-1)', borderRadius: 4, padding: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{ height: 8, background: theme === 'dark' ? '#222831' : 'var(--line)', borderRadius: 2, width: '40%' }}/>
        {[80,65,90,55,72,48].map((w,i) => <div key={i} style={{ height: 6, background: theme === 'dark' ? '#1c2127' : 'var(--line)', borderRadius: 2, width: `${w}%` }}/>)}
      </div>
    </div>
  </div>
);

const WsTerminalMock = ({ theme = 'light' }) => (
  <div style={{ background: theme === 'dark' ? '#0b0d10' : '#1a1d20', borderRadius: 6, padding: 10, fontFamily: 'var(--font-mono)', fontSize: 10.5, color: '#9aa4b2', display: 'flex', flexDirection: 'column', gap: 2, height: '100%', overflow: 'hidden', border: '1px solid var(--line)' }}>
    <div style={{ color: '#7ee787' }}>$ pnpm dev</div>
    <div>▲ Next.js 14.2.3</div>
    <div>  - Local:        <span style={{ color: '#79c0ff' }}>http://localhost:13180</span></div>
    <div>  - Environments: .env.local</div>
    <div style={{ color: '#a5d6ff' }}> ✓ Ready in 1.4s</div>
    <div> ○ Compiling /admin/dashboard ...</div>
    <div style={{ color: '#a5d6ff' }}> ✓ Compiled /admin/dashboard in 380ms (1247 modules)</div>
    <div> GET /admin/dashboard 200 in 412ms</div>
    <div> GET /api/health 200 in 24ms</div>
    <div style={{ color: '#7ee787' }}>$ <span style={{ background: '#9aa4b2', display: 'inline-block', width: 6, height: 11, verticalAlign: 'middle' }}/></div>
  </div>
);

const WsFilesMock = () => (
  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, padding: 8, display: 'flex', flexDirection: 'column', gap: 2 }}>
    {[['📁','services'],['📁','schemas'],['📁','jobs'],['📄','package.json',' M'],['📄','tsconfig.json'],['📁','tests'],['📄','README.md',' M'],['📄','docker-compose.yml']].map(([icon,name,mod],i) => (
      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '2px 4px', color: 'var(--ink-2)' }}>
        <span style={{ fontSize: 10 }}>{icon}</span>
        <span style={{ flex: 1 }}>{name}</span>
        {mod && <span style={{ color: 'var(--info)', fontWeight: 600 }}>{mod}</span>}
      </div>
    ))}
  </div>
);
...
// ============================================================
// V1 — Master/Detail split. List left, live workspace right.
// ============================================================
const WorkspacesV1 = () => {
  ...
  return (
    <V1Frame active="workspaces" crumbs={['Develop', 'Workspaces']}
      actions={<><span className="mono" style={{ fontSize: 11, color: 'var(--ink-3)' }}>11 ACTIVE · 1 OPEN</span><button className="btn btn-primary"><Icon name="plus" size={11}/> NEW</button></>}>
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        {/* LEFT — workspace list */}
        <div style={{ width: 360, borderRight: '1px solid var(--line)', display: 'flex', flexDirection: 'column', background: 'var(--bg-0)' }}>
          <div style={{ padding: '8px 12px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-1)' }}>
            ...
            <input placeholder="Filter sandboxes…" style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--ink-2)' }}/>
            <span className="mono" style={{ fontSize: 10, color: 'var(--ink-3)' }}>{workspaces.length}</span>
          </div>
          <div style={{ padding: '6px 12px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 6, background: 'var(--bg-1)' }}>
            <Icon name="repo" size={10} color="var(--acc)"/>
            <span className="mono" style={{ fontSize: 10.5, fontWeight: 600 }}>es-data-pipeline</span>
            <span style={{ fontSize: 10, color: 'var(--ink-3)' }}>· 9</span>
          </div>
          <div style={{ flex: 1, overflow: 'auto' }}>
            {workspaces.map((ws, i) => {
              const active = i === selected;
              return (
                <div key={i} onClick={() => setSelected(i)} style={{ padding: '8px 12px', borderBottom: '1px solid var(--line)', cursor: 'pointer', background: active ? 'var(--bg-1)' : 'transparent', borderLeft: active ? '2px solid var(--acc)' : '2px solid transparent', display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Icon name="merge" size={9} color={active ? 'var(--acc)' : 'var(--ink-3)'}/>
                    <span className="mono" style={{ fontSize: 11, fontWeight: active ? 600 : 500, color: active ? 'var(--ink)' : 'var(--ink-2)', flex: 1 }} className="truncate">{ws[0]}</span>
                    {ws[4].includes('running') && <span style={{ width: 6, height: 6, borderRadius: 6, background: 'var(--ok)' }}/>}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 9.5, color: 'var(--ink-3)' }}>
                    <span className="mono">:{ws[3]}</span>
                    {ws[5] != null && <span className="mono" style={{ color: 'var(--info)' }}>+{ws[5]}</span>}
                    {ws[4].filter(s => s.startsWith('PR')).map((s, j) => <span key={j} className="pill pill-warn">{s}</span>)}
                  </div>
                </div>
              );
            })}
        ...
        {/* RIGHT — workspace detail (lives in same view) */}
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: 'var(--bg-0)' }}>
          <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 10, background: 'var(--bg-1)' }}>
            <Icon name="box" size={13} color="var(--acc)"/>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="mono" style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink)' }} className="truncate">{w[0]}</div>
              <div className="mono" style={{ fontSize: 10, color: 'var(--ink-3)', marginTop: 2 }} className="truncate">{w[2]} · :{w[3]}</div>
            </div>
            {w[4].map((s, j) => <span key={j} className={`pill ${s === 'running' ? 'pill-run' : s.startsWith('PR') ? 'pill-warn' : 'pill-ok'}`}>{s}</span>)}
            <button className="btn btn-line" style={{ padding: '3px 8px', fontSize: 10 }}><Icon name="github" size={10}/> PR</button>
            <button className="btn btn-line" style={{ padding: '3px 8px', fontSize: 10 }}>↗ Pop out</button>
          </div>
          <div style={{ borderBottom: '1px solid var(--line)', background: 'var(--bg-1)', display: 'flex', gap: 0, paddingLeft: 8 }}>
            {[['preview','Preview'],['terminal','Terminal'],['files','Files'],['logs','Logs'],['env','Env']].map(([id, label]) => (
              <div key={id} onClick={() => setTab(id)} style={{ padding: '7px 12px', fontSize: 11, fontWeight: 500, color: tab === id ? 'var(--ink)' : 'var(--ink-3)', borderBottom: tab === id ? '2px solid var(--acc)' : '2px solid transparent', cursor: 'pointer' }}>{label}</div>
            ))}
          </div>
          <div style={{ flex: 1, padding: 12, minHeight: 0, display: 'flex' }}>
            {tab === 'preview' && <WsPreviewMock port={w[3]}/>}
            {tab === 'terminal' && <WsTerminalMock/>}
            {tab === 'files' && <div style={{ flex: 1, background: 'var(--bg-1)', border: '1px solid var(--line)', borderRadius: 6, overflow: 'auto' }}><WsFilesMock/></div>}
            {tab === 'logs' && <WsTerminalMock/>}
            {tab === 'env' && <div style={{ flex: 1, background: 'var(--bg-1)', border: '1px solid var(--line)', borderRadius: 6, padding: 12, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--ink-2)' }}>
              <div>DATABASE_URL=<span style={{ color: 'var(--ink-3)' }}>postgresql://...</span></div>
              <div>NODE_ENV=<span style={{ color: 'var(--ink-3)' }}>development</span></div>
              <div>OPENSEARCH_HOST=<span style={{ color: 'var(--ink-3)' }}>localhost:9200</span></div>
              <div>PORT=<span style={{ color: 'var(--ink-3)' }}>{w[3]}</span></div>
            </div>}
          </div>
        </div>
      </div>
...
// ============================================================
// V2 — Inline expand. Click a row, it expands in place.
// ============================================================
const WorkspacesV2 = () => {
  const [expanded, setExpanded] = React.useState(2); // pre-expanded for demo
  const [tab, setTab] = React.useState('preview');
  return (
    <V2Frame active="workspaces" title="Sandboxes" crumbs={['Code', 'Sandboxes']}
      ...
      <div style={{ flex: 1, padding: '14px 24px 24px', overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {workspaces.slice(0, 6).map((w, i) => {
            ...
            return (
              <div key={i} style={{ background: 'var(--bg-1)', border: '1px solid var(--line)', borderRadius: 10, overflow: 'hidden', boxShadow: isOpen ? '0 1px 3px rgba(0,0,0,0.04)' : 'none' }}>
                {/* row header */}
                <div onClick={() => setExpanded(isOpen ? -1 : i)} style={{ padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                  <span style={{ display: 'inline-block', transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.15s', fontSize: 10, color: 'var(--ink-3)' }}>▶</span>
                  <Icon name="box" size={13} color="var(--acc)"/>
                  <span className="mono" style={{ flex: 1, fontSize: 12.5, fontWeight: 500 }} className="truncate">{w[0]}</span>
                  {w[5] != null && <span className="mono" style={{ fontSize: 11, color: 'var(--info)' }}>+{w[5]} changed</span>}
                  <span className="mono" style={{ fontSize: 11, color: 'var(--ink-3)' }}>:{w[3]}</span>
                  {w[4].map((s, j) => <span key={j} className={`pill ${s === 'running' ? 'pill-run' : s.startsWith('PR') ? 'pill-warn' : 'pill-ok'}`}>{s}</span>)}
                  {!isOpen && <button className="btn btn-line" style={{ padding: '3px 10px', fontSize: 11 }} onClick={(e) => { e.stopPropagation(); setExpanded(i); }}>Open</button>}
                </div>
                {/* expanded panel */}
                {isOpen && (
                  <div style={{ borderTop: '1px solid var(--line)', background: 'var(--bg-0)', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 6, background: 'var(--bg-1)' }}>
                      {[['preview','Preview'],['terminal','Terminal'],['files','Files'],['env','Env']].map(([id,label]) => (
                        <button key={id} onClick={() => setTab(id)} className={`btn ${tab === id ? 'btn-primary' : 'btn-line'}`} style={{ padding: '3px 10px', fontSize: 11 }}>{label}</button>
                      ))}
                      <span style={{ flex: 1 }}/>
                      <span className="mono" style={{ fontSize: 10, color: 'var(--ink-3)' }} className="truncate">{w[2]}</span>
                      <button className="btn btn-line" style={{ padding: '3px 8px', fontSize: 10 }}>↗ New tab</button>
                      <button className="btn btn-line" style={{ padding: '3px 8px', fontSize: 10 }}>Close ✕</button>
                    </div>
                    <div style={{ height: 320, padding: 12, display: 'flex', gap: 10, minHeight: 0 }}>
                      {tab === 'preview' && <div style={{ flex: 1 }}><WsPreviewMock port={w[3]}/></div>}
                      {tab === 'terminal' && <div style={{ flex: 1 }}><WsTerminalMock/></div>}
                      {tab === 'files' && <div style={{ flex: 1, background: 'var(--bg-1)', border: '1px solid var(--line)', borderRadius: 6, overflow: 'auto' }}><WsFilesMock/></div>}
                      {tab === 'env' && <div style={{ flex: 1, background: 'var(--bg-1)', border: '1px solid var(--line)', borderRadius: 6, padding: 12, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--ink-2)' }}>
...
// ============================================================
// V3 — Tabbed dock. Multiple sandboxes open as tabs above table.
// ============================================================
const WorkspacesV3 = () => {
  const [openTabs, setOpenTabs] = React.useState([0, 4, 5]); // pre-opened
  const [active, setActive] = React.useState(0);
  const [pane, setPane] = React.useState('preview');
  const w = workspaces[active];
  const isOpen = (i) => openTabs.includes(i);
  return (
    <V3Frame active="workspaces" title="Sandboxes" subtitle="open in place · multi-tab" count={`${workspaces.length} ACTIVE · ${openTabs.length} OPEN`}
      actions={<><button className="btn btn-line mono">GROUP: REPO</button><button className="btn btn-primary mono">+ SANDBOX</button></>}>
      {/* Tab bar — open sandboxes */}
      <div style={{ borderBottom: '2px solid var(--ink)', background: 'var(--bg-1)', display: 'flex', alignItems: 'stretch', minHeight: 36 }}>
        {openTabs.map((idx) => {
          const ws = workspaces[idx];
          const isActive = idx === active;
          return (
            <div key={idx} onClick={() => setActive(idx)} style={{ padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontFamily: 'var(--font-mono)', borderRight: '1px solid var(--ink)', background: isActive ? 'var(--bg-0)' : 'transparent', borderTop: isActive ? '2px solid var(--acc)' : '2px solid transparent', marginTop: isActive ? -2 : 0, cursor: 'pointer', maxWidth: 240, fontWeight: isActive ? 600 : 400 }}>
              <span style={{ width: 6, height: 6, borderRadius: 6, background: ws[4].includes('running') ? 'var(--ok)' : 'var(--info)' }}/>
              <span className="truncate" style={{ flex: 1 }}>{ws[0].slice(0, 28)}</span>
              <span style={{ color: 'var(--ink-3)' }}>:{ws[3]}</span>
              <span onClick={(e) => { e.stopPropagation(); setOpenTabs(openTabs.filter(t => t !== idx)); if (active === idx) setActive(openTabs.find(t => t !== idx) || 0); }} style={{ color: 'var(--ink-3)', padding: '0 2px' }}>✕</span>
            </div>
          );
        })}
        <div style={{ padding: '8px 12px', fontSize: 14, color: 'var(--ink-3)', cursor: 'pointer' }}>+</div>
      </div>

      {/* Active workspace pane */}
      {w && (
        <div style={{ background: 'var(--bg-0)', borderBottom: '1px solid var(--ink)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid var(--line)', background: 'var(--bg-1)' }}>
            <span className="mono" style={{ fontSize: 10, color: 'var(--ink-3)', letterSpacing: '0.06em' }}>ACTIVE/</span>
            <span className="mono" style={{ fontSize: 12, fontWeight: 700 }} className="truncate">{w[0]}</span>
            <span className="mono" style={{ fontSize: 10, color: 'var(--ink-3)' }} className="truncate">{w[2]}</span>
            <span style={{ flex: 1 }}/>
            {[['preview','PREVIEW'],['terminal','TERMINAL'],['files','FILES']].map(([id,label]) => (
              <button key={id} onClick={() => setPane(id)} className="btn btn-line mono" style={{ padding: '3px 10px', fontSize: 10, background: pane === id ? 'var(--ink)' : 'transparent', color: pane === id ? 'var(--bg-0)' : 'var(--ink)' }}>{label}</button>
            ))}
            <button className="btn btn-primary mono" style={{ padding: '3px 10px', fontSize: 10 }}>↗ POP OUT</button>
          </div>
          <div style={{ height: 280, padding: 10, display: 'flex', gap: 8 }}>
            {pane === 'preview' && <div style={{ flex: 1 }}><WsPreviewMock port={w[3]}/></div>}
            {pane === 'terminal' && <div style={{ flex: 1 }}><WsTerminalMock/></div>}
            {pane === 'files' && <div style={{ flex: 1, background: 'var(--bg-1)', border: '1px solid var(--line)', borderRadius: 4, overflow: 'auto' }}><WsFilesMock/></div>}
          </div>
        </div>
      )}

      {/* Sandbox table — click row to open as new tab */}
      <div style={{ flex: 1, overflow: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-mono)', fontSize: 11.5 }}>
          <thead><tr style={{ background: 'var(--bg-1)', borderBottom: '1px solid var(--ink)', position: 'sticky', top: 0 }}>
            {['', 'NAME', 'BRANCH', 'PORT', 'CHG', 'STATUS', ''].map((h, i) => <th key={i} style={{ padding: '8px 10px', textAlign: 'left', fontSize: 10, color: 'var(--ink-3)', letterSpacing: '0.08em', borderRight: i < 6 ? '1px solid var(--line)' : 'none' }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {workspaces.map((ws, i) => {
              const open = isOpen(i);
              const isActive = i === active && open;
              return (
                <tr key={i} style={{ borderBottom: '1px solid var(--line)', background: isActive ? 'var(--bg-1)' : 'var(--bg-0)' }}>
                  <td style={{ padding: '6px 10px', borderRight: '1px solid var(--line)', width: 28 }}>
                    {open ? <span style={{ width: 6, height: 6, borderRadius: 6, background: 'var(--acc)', display: 'inline-block' }}/> : <input type="checkbox"/>}
                  </td>
                  <td style={{ padding: '6px 10px', fontWeight: isActive ? 700 : 600, borderRight: '1px solid var(--line)', maxWidth: 0 }} className="truncate">{ws[0]}</td>
                  <td style={{ padding: '6px 10px', color: 'var(--acc)', borderRight: '1px solid var(--line)', maxWidth: 220 }} className="truncate">{ws[2]}</td>
                  <td style={{ padding: '6px 10px', borderRight: '1px solid var(--line)' }}>{ws[3]}</td>
                  <td style={{ padding: '6px 10px', color: 'var(--info)', borderRight: '1px solid var(--line)' }}>{ws[5] || '—'}</td>
                  <td style={{ padding: '6px 10px', borderRight: '1px solid var(--line)' }}>{ws[4].join(' ')}</td>
                  <td style={{ padding: '4px 8px', width: 90 }}>
                    {open ? (
                      <button onClick={() => setActive(i)} className="btn btn-line mono" style={{ fontSize: 10, background: isActive ? 'var(--ink)' : 'transparent', color: isActive ? 'var(--bg-0)' : 'var(--ink)' }}>{isActive ? 'ACTIVE' : 'FOCUS'}</button>
                    ) : (
                      <button onClick={() => { setOpenTabs([...openTabs, i]); setActive(i); }} className="btn btn-line mono" style={{ fontSize: 10 }}>+ OPEN</button>
                    )}
                  </td>
                </tr>
              );
