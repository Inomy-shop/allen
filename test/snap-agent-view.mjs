// Snap agents page + open agent view dialog.
...
(async () => {
  ...
  await page.screenshot({ path: join(OUT, 'agents-no-sidebar.png') });
  console.log('list saved');

  // Click first agent card
  const firstAgentCard = page.locator('button.card-hover').nth(2);
  if (await firstAgentCard.count()) {
    await firstAgentCard.click();
    await page.waitForTimeout(700);
    await page.screenshot({ path: join(OUT, 'agents-view-dialog.png') });
