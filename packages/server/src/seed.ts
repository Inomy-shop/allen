...
Return evidence checked, module summary, data/control flow, important dependencies, unknowns, and suggested next step.`,
  },
  {
    ...
    relatedAgents: ['allen-workflow-diagnostician', 'allen-tooling-diagnostician', 'bug-investigator'],
...
State the observed status, evidence checked, root cause or likely reason, and the next operational action.`,
  },
  {
    ...
    triggers: ['linear', 'ticket', 'issue', 'sprint', 'backlog', 'closed today', 'assign ticket', 'dispatch ticket'],
    excludes: ['github pr review', 'review pr', 'fix bug', 'implement feature'],
    priority: 86,
...
Return evidence checked, findings, confidence, unknowns, and recommended route if a fix is needed.`,
  },
  {
    ...
    triggers: ['fix bug', 'bug', 'broken', 'not working', 'regression', 'production issue', 'crash', 'error'],
    excludes: ['add feature', 'new workflow', 'create team', 'create agent'],
    priority: 82,
...
State selected route, why direct specialist or workflow is appropriate, required inputs, workspace/PR expectation, and any missing context.`,
  },
  {
    ...
    triggers: ['implement', 'feature', 'new feature', 'build a feature', 'add a feature', 'add support for', 'revamp', 'redesign', 'enhance'],
    excludes: ['fix bug', 'resolve pr comments', 'investigate', 'review pr'],
    priority: 80,
...
Return findings by severity, evidence checked, route chosen, and PR/workspace links when applicable.`,
  },
  {
    ...
    excludes: ['read only', 'explain only', 'view workspace', 'workspace status', 'show workspace'],
    priority: 76,
...
Return: the builder agent you chose (as discovered at runtime), the blueprint preview verbatim from the builder when available, the created record IDs and names, and a clickable link to each new team/agent/workflow/skill when the UI route is known. Never paste raw create_* outputs without context.`,
  },
  {
    ...
    triggers: ['assign', 'delegate', 'route to', 'lead', 'team', '@agent', 'handoff', 'hand off to', 'have the team'],
    excludes: ['create team', 'build team', 'new team', 'add team', 'create agent', 'create workflow', 'add skill', 'edit skill', 'run workflow'],
    priority: 72,
