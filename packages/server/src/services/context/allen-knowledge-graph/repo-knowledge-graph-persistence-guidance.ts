...
<${REPO_KNOWLEDGE_GRAPH_PERSISTENCE_GUIDANCE_SENTINEL} priority="critical">
  <rule>When this task asks you to index, save, persist, refresh, recreate, rebuild, or replace a repo knowledge graph, the task MUST specify mode full_graph or mandatory_context_map. If the mode is missing, stop and report that mode is required.</rule>
  <rule>When mode is specified, you MUST call the Allen MCP tool save_repo_knowledge_graph, shown by Claude as mcp__allen__save_repo_knowledge_graph when namespaced.</rule>
  <rule>Pass repo_path, graph_mode, and the full generated graph as graph or graph_json.${repoPathLine}</rule>
