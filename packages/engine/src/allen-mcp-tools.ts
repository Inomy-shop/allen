import { MCP_SERVER_NAME } from './brand.js';

export const ALLEN_MCP_TOOL_NAMES: readonly string[] = [
  'list_skills',
  'search_skills',
  'get_skill',
  'list_workflows',
  'get_workflow',
  'run_workflow',
  'validate_workflow',
  'create_workflow',
  'update_workflow',
  'wait_for_execution',
  'list_executions',
  'search_executions',
  'cancel_execution',
  'resume_execution',
  'submit_execution_input',
  'get_node_trace',
  'get_execution_logs',
  'list_agents',
  'get_agent',
  'create_agent',
  'update_agent',
  'delete_agent',
  'move_agent_to_team',
  'spawn_agent',
  'list_teams',
  'get_team',
  'get_team_blueprint',
  'list_team_members',
  'create_team',
  'update_team',
  'delete_team',
  'list_repos',
  'get_repo_context',
  'get_repo_context_body',
  'get_repo_skill_body',
  'get_node_context_usage',
  'context_quality_get_usage_trace',
  'context_quality_replay_usage_trace',
  'context_quality_get_attempt_evidence',
  'context_quality_get_attempt_evidence_batch',
  'context_quality_trigger_orchestrator',
  'context_quality_begin_session',
  'context_quality_log_decision',
  'context_quality_submit_findings',
  'context_quality_finalize_session',
  'context_quality_get_session',
  'context_quality_get_stage_state',
  'context_quality_get_repair_state',
  'context_quality_list_pending',
  'context_quality_list_findings',
  'context_quality_log_source_evaluation',
  'context_quality_submit_source_evaluation',
  'context_quality_list_unevaluated_traces',
  'context_quality_create_trace_analysis_assignment',
  'context_quality_create_trace_analysis_wave',
  'context_quality_list_trace_analysis_assignments',
  'context_quality_get_trace_analysis_assignment',
  'context_quality_update_trace_analysis_assignment',
  'context_quality_list_source_evaluations',
  'context_quality_update_worker_assignment',
  'context_quality_create_worker_assignment',
  'context_quality_patch_finding',
  'context_quality_list_remediation_tasks',
  'context_quality_create_remediation_task',
  'context_quality_dispatch_remediation_task',
  'context_quality_create_learning_promotion',
  'context_quality_decide_learning_promotion',
  'context_quality_apply_curated_edit',
  'context_quality_get_curated_entry',
  'context_quality_get_curation_history',
  'context_quality_revert_curated_edit',
  'prepare_repo_context_curation',
  'plan_repo_context_curation_assignments',
  'register_repo_context_curation_assignments',
  'get_repo_context_curation_stage_status',
  'promote_repo_context_curation_stage',
  'save_repo_context_curation_stage',
  'save_repo_mandatory_context_mappings',
  'find_repo_for_pr_url',
  'find_pr_by_url',
  'mark_pr_synced',
  'get_workspace',
  'create_workspace_for_pr',
  'create_workspace',
  'ask_user',
  'report_to_user',
  'get_my_session_history',
  'get_chat_session',
  'get_chat_messages',
  'get_chat_logs',
  'get_chat_log',
  'allen_monitoring_get_scan_cursor',
  'allen_monitoring_update_scan_cursor',
  'allen_monitoring_search_records',
  'allen_monitoring_get_record',
  'allen_monitoring_create_evidence_bundle',
  'allen_monitoring_search_incidents',
  'allen_monitoring_upsert_incident',
  'allen_monitoring_update_incident',
  'allen_monitoring_resolve_repo_path',
  'search_learnings',
  'save_learning',
  'get_dashboard_stats',
  'query_database',
  'upload_file',
  'allen_save_artifact',
  'allen_list_artifacts',
  'allen_get_artifact',
];

export const ALLEN_MCP_CLAUDE_TOOL_NAMES: readonly string[] = ALLEN_MCP_TOOL_NAMES.map(
  (tool) => `mcp__${MCP_SERVER_NAME}__${tool}`,
);
