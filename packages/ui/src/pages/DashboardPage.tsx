...
export default function DashboardPage() {
  ...
  if (loading) {
    return (
      <div className="p-6">
        <h1 className="font-heading text-xl font-bold text-theme-primary mb-6 tracking-widest uppercase">Dashboard</h1>
    ...
    <div className="p-6">
        ...
        {cards.map(card => {
          ...
          return (
            <div key={card.label} className={`card p-4 hover:${card.glow} transition-shadow duration-300`}>
              ...
              <div className="text-2xl font-heading font-bold text-theme-primary tabular-nums">{card.value}</div>
              <div className="text-xs text-theme-muted mt-0.5 font-label uppercase tracking-wider">{card.label}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-4">
          <h2 className="font-heading text-sm font-semibold text-theme-primary mb-4 flex items-center gap-2 tracking-wider uppercase">
            <Clock className="w-4 h-4 text-theme-muted" />
            Avg Duration by Workflow
          </h2>
          {Object.keys(avgDuration).length === 0 ? (
            <p className="text-xs text-theme-subtle font-mono">NO DATA YET</p>
          ) : (
            <div className="space-y-3">
              {Object.entries(avgDuration).map(([name, d]: [string, any]) => (
                <div key={name} className="flex items-center justify-between">
                  <span className="text-sm text-theme-secondary font-body">{name}</span>
                  <div className="text-right">
                    <span className="text-sm text-theme-primary tabular-nums font-mono">
                      {(d.avgDuration / 1000).toFixed(1)}s
                    </span>
                    <span className="text-xs text-theme-subtle ml-2 font-mono">
                      ({d.count} runs)
                    </span>
                  </div>
                </div>
