    ...
    <div className="mx-4 mb-4 rounded-lg border border-accent-cyan/30 bg-accent-cyan/5 overflow-hidden">
      ...
      <div className="px-4 pb-3 flex gap-2">
        <input
          ...
          autoFocus
          disabled={submitting}
          className="flex-1 bg-surface-200/50 border border-border/30 rounded-md px-3 py-2 text-sm text-white placeholder-gray-600 font-body focus:outline-none focus:border-accent-cyan/50 disabled:opacity-50"
        />
        <button
          onClick={handleSubmit}
          disabled={!answer.trim() || submitting}
          className="shrink-0 px-4 py-2 rounded-md bg-accent-cyan/20 text-accent-cyan text-sm font-mono hover:bg-accent-cyan/30 transition-colors disabled:opacity-30 disabled:cursor-not-allowed flex items-center gap-1.5"
        >
