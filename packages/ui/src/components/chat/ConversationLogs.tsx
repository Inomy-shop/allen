...
const TYPE_STYLE: Record<string, { icon: React.ElementType; color: string; label: string }> = {
  ...
  complete: { icon: CheckCircle, color: 'text-gray-500', label: 'Complete' },
};

function TraceEventRow({ event }: { event: TraceEvent }) {
  const [expanded, setExpanded] = useState(false);
  const style = TYPE_STYLE[event.type] ?? { icon: MessageSquare, color: 'text-gray-500', label: event.type };
  ...
  return (
    <div className={`border-l-2 ${event.isError ? 'border-accent-red/50' : 'border-border/30'} ml-3`}>
      ...
      >
        <span className="text-[10px] font-mono text-gray-600 w-16 shrink-0">{time}</span>
        ...
        {toolName && <span className="text-[11px] font-mono text-gray-400">{toolName}</span>}
        {event.durationMs != null && <span className="text-[10px] font-mono text-gray-600">{event.durationMs}ms</span>}
        {event.text && !event.tool && <span className="text-[10px] text-gray-500 truncate max-w-[200px]">{event.text.slice(0, 80)}</span>}
        <span className="flex-1" />
        {hasDetail && (expanded ? <ChevronDown className="w-3 h-3 text-gray-600" /> : <ChevronRight className="w-3 h-3 text-gray-600" />)}
      </button>

      {expanded && (
        <div className="mx-3 mb-2 ml-8 p-2 bg-[rgb(13,17,28)] rounded-md border border-border/20 max-h-60 overflow-auto">
          {event.args && Object.keys(event.args).length > 0 && (
            <div className="mb-2">
              <span className="text-[9px] font-label uppercase tracking-widest text-gray-600">Args</span>
              <pre className="text-[11px] font-mono text-gray-400 mt-0.5 whitespace-pre-wrap">{JSON.stringify(event.args, null, 2)}</pre>
            </div>
          )}
          {event.result && (
            <div>
              <span className="text-[9px] font-label uppercase tracking-widest text-gray-600">Result</span>
              <pre className="text-[11px] font-mono text-gray-400 mt-0.5 whitespace-pre-wrap">{JSON.stringify(event.result, null, 2).slice(0, 2000)}</pre>
            </div>
          )}
          {event.text && event.text.length > 80 && !event.args && !event.result && (
            <pre className="text-[11px] font-mono text-gray-400 whitespace-pre-wrap">{event.text.slice(0, 1000)}</pre>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className="border border-border/30 rounded-lg overflow-hidden mb-3">
      ...
      >
        ...
        <span className="text-xs text-gray-300 font-body truncate flex-1">{log.userMessage?.slice(0, 60) ?? '(no message)'}</span>
        <span className="text-[10px] font-mono text-gray-600">{toolCount} tools</span>
        <span className="text-[10px] font-mono text-gray-600">{(log.durationMs / 1000).toFixed(1)}s</span>
        {log.costUsd > 0 && <span className="text-[10px] font-mono text-gray-600">${log.costUsd.toFixed(4)}</span>}
      ...
      {expanded && (
        <div className="py-1">
          {(log.trace ?? []).length > 0 ? (
            ...
            <div className="px-4 py-2 text-[11px] text-gray-600">No trace events recorded</div>
          )}

          {log.assistantResponse && (
            <div className="mx-3 mt-2 mb-2 p-2 bg-surface-200/20 rounded-md border border-border/20">
              <span className="text-[9px] font-label uppercase tracking-widest text-gray-600">Response preview</span>
              <p className="text-[11px] text-gray-400 font-body mt-1 line-clamp-3">{log.assistantResponse.slice(0, 300)}</p>
            </div>
          )}
        </div>
      )}
    ...
    <div className="fixed top-0 right-0 bottom-0 left-0 z-50 flex justify-end" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border/40 bg-surface-50">
          <div>
            <h2 className="font-heading text-sm text-white tracking-wider">{session?.title ?? 'Conversation Logs'}</h2>
            <div className="flex items-center gap-3 mt-0.5">
              <span className="text-[10px] text-gray-500 font-mono">{session?.provider}</span>
              <span className="text-[10px] text-gray-500 font-mono">{logs.length} messages</span>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading && <div className="text-xs text-gray-600 animate-pulse">Loading logs...</div>}
          {!loading && logs.length === 0 && <div className="text-xs text-gray-600">No logs recorded for this conversation.</div>}
