...
// ===== artifacts (PRDs, TDDs, reviews, postmortems) =====
// Each artifact references a markdown file in the prototype dir; viewer fetches + renders.
const ARTIFACTS = {
  'art-prd-render': {
    id: 'art-prd-render',
    taskId: 't-render-wait',
    type: 'PRD',
    title: 'Render Wait Experience — Style Preference Collection',
    status: 'draft',
    version: 'v1',
    edited_at: '4m ago',
    author: 'allen',
    file: 'render_wait_explore_prd.md',
    summary: '3–4 minute render wait → active style-preference surface. Reduces abandonment, builds cold-start signal.',
  },
};
function artifactsForTask(taskId) {
  return Object.values(ARTIFACTS).filter(a => a.taskId === taskId);
}
...
const TASK_FIXTURES = {
  ...
  't-render-wait': {
    id: 't-render-wait', ticket: 'ENG-1517', title: 'Render Wait Experience — Style Preference Collection',
    repo: 'inomy-mono', workflow: 'feature-plan-and-implement', owner: 'you',
    status: 'planning', progress: 25, eta: 'on you', cost: 1.84, branch: '—',
    pr: null,
    chat: [
      { who: 'you',   ts: '4:42 PM', text: "We have a 3-4 minute render wait when a user submits a room redesign. Today it's a static placeholder. Let's design a wait-state surface that turns it into a preference-collection moment — users browse curated rooms, react with like/dislike/save, and a style profile gets built in real time. Produce a PRD." },
      { who: 'allen', ts: '4:43 PM', text: 'Drafting the PRD. Will cover: goals, layout (left panel transforms; right chat panel stays), card stack, profile viz, render-ready handoff, edge cases, success metrics. Pulling in past style-recommendation work as reference.' },
      { who: 'allen', ts: '4:46 PM', kind: 'doc-card', artifactId: 'art-prd-render' },
      { who: 'allen', ts: '4:46 PM', text: '`PRD v1` published. Three open questions flagged in §9 — please review. Top one: should Explore be reachable outside the wait state in v1? My recommendation is yes (low effort, high optionality), but I want your call before HLA.' },
    ],
    steps: [
      { id: 'intake',  name: 'intake & clarify',  state: 'ok',       actor: 'intake-clarifier', dur: '8s',  cost: 0.04 },
      { id: 'prd',     name: 'PRD v1',            state: 'ok',       actor: 'prd-writer',       dur: '1m 12s', cost: 0.32, sub: 'render_wait_explore_prd.md' },
      { id: 'gate',    name: 'PRD review',        state: 'wait-you', actor: 'you',              dur: '—',   cost: 0,    sub: '3 open questions in §9' },
      { id: 'hla',     name: 'HLA',               state: 'wait',     actor: 'hla-writer',       dur: '—',   cost: 0 },
...
const THREAD_FIXTURES = [
  // ongoing
  { id: 't-render-wait', title: 'Render-wait experience PRD', sub: 'feature-plan-and-implement', when: '4m ago', status: 'planning', progress: 25, ticket: 'ENG-1517', repo: 'inomy-mono', pr: null },
...
function TaskPage({ taskId, setRoute, openTask }) {
  ...
  const [agent, setAgent] = nUseState('assistant');
  const [viewingArtifact, setViewingArtifact] = nUseState(null);
  const taskArtifacts = artifactsForTask(t.id);
  ...
  return (
    <div className={`chat-page ${railOpen?'rail-open':'rail-closed'}`} data-screen-label="task">
      <main className="chat-main">
        ...
        {viewingArtifact ? (
          <ArtifactReader artifactId={viewingArtifact} onClose={()=>setViewingArtifact(null)}/>
        ) : (
          <div className="chat-stream">
            {stream.map((m, i) => <StreamItem key={i} m={m} t={t} onOpenArtifact={setViewingArtifact}/>)}
          </div>
        )}
        ...
        <aside className="chat-rail">
          ...
          {taskArtifacts.length > 0 && (
            <section className="cr-section">
              <h6>artifacts <span className="cr-ct">{taskArtifacts.length}</span></h6>
              {taskArtifacts.map(a => (
                <button key={a.id} className={`cr-art ${viewingArtifact===a.id?'active':''}`} onClick={()=>setViewingArtifact(a.id)}>
                  <span className="cr-art-ic"><Icons.file size={11}/></span>
                  <span className="cr-art-body">
                    <span className="cr-art-h">
                      <span className="cr-art-tag">{a.type.toLowerCase()}</span>
                      <span className="cr-art-v mono">{a.version}</span>
                    </span>
                    <span className="cr-art-title">{a.title}</span>
                    <span className="cr-art-sub">edited {a.edited_at}</span>
                  </span>
                  <Icons.chevR size={11}/>
                </button>
              ))}
          ...
          <section className="cr-section">
            ...
            {t.branch && t.branch !== '—' && (
              <a className="cr-ref ws" href="#" onClick={(e)=>{ e.preventDefault(); setRoute('workspaces'); }}>
                <span className="cr-ref-ic ws">⎈</span>
                <span className="cr-ref-body">
                  <span className="cr-ref-id">workspace <span className="cr-ref-tag open">live</span></span>
                  <span className="cr-ref-sub">terminal · diff · file tree →</span>
                </span>
                <Icons.chevR size={11}/>
              </a>
            )}
...
function buildStream(t) {
  ...
  for (let i = 1; i < t.chat.length; i++) {
    const m = t.chat[i];
    if (m.kind === 'doc-card') {
      out.push({ kind:'doc-card', ts:m.ts, who:m.who, artifactId:m.artifactId });
      continue;
    }
...
// Inline doc-card — appears in chat when an artifact is produced
function DocCard({ artifactId, onOpen }) {
  ...
  return (
    <div className="ch-msg allen">
      ...
      <div className="ch-msg-body">
        <button className="ch-card doc-card" onClick={()=>onOpen(artifactId)}>
          <div className="ch-card-h">
            <span className="cc-tag doc">{a.type.toLowerCase()}</span>
            <span className="cc-title">{a.title}</span>
            <span className="cc-pct mono">{a.version}</span>
          </div>
          <div className="ch-card-b">
            <p className="dc-summary">{a.summary}</p>
            <div className="dc-meta">
              <span className="dc-meta-i"><Icons.file size={11}/> {a.file}</span>
              <span className="dc-meta-i muted">edited {a.edited_at}</span>
              <span className="dc-meta-i" style={{marginLeft:'auto', color:'var(--accent)'}}>open document →</span>
            </div>
          </div>
        </button>
      </div>
...
// ArtifactReader — replaces the chat-stream when an artifact is being read.
// Loads the .md file, renders with marked.parse, styles with the prose token set.
function ArtifactReader({ artifactId, onClose }) {
  const a = ARTIFACTS[artifactId];
  const [md, setMd] = nUseState('');
  const [err, setErr] = nUseState(null);
  ...
  // Use marked from the CDN — fall back to a minimal renderer if not loaded yet.
  const html = nUseMemo(() => {
    if (!md) return '';
    if (window.marked) return window.marked.parse(md);
    // tiny fallback — escape and wrap; user will see plaintext until marked loads
    return '<pre>'+md.replace(/[<>&]/g, c => ({'<':'&lt;','>':'&gt;','&':'&amp;'}[c]))+'</pre>';
  }, [md]);

  if (!a) {
    return <div className="art-reader"><div className="task-empty">artifact not found</div></div>;
  }

  return (
    <div className="art-reader" data-screen-label="artifact">
      <header className="ar-head">
        <button className="link" onClick={onClose}><Icons.chevL size={11}/> back to chat</button>
        <span className="ar-bc-d">·</span>
        <span className="ar-type">{a.type}</span>
        <span className="ar-bc-d">/</span>
        <span className="ar-title">{a.title}</span>
        <span className="ar-version mono">{a.version}</span>
        <div style={{flex:1}}/>
        <button className="btn ghost sm"><Icons.download size={11}/> download</button>
        <button className="btn ghost sm"><Icons.edit size={11}/> edit</button>
        <button className="btn primary sm"><Icons.chat size={11}/> comment</button>
      </header>
      <div className="ar-body">
        {err && <div className="ar-err">couldn't load <code>{a.file}</code> — {err}. Hard-refresh and try again.</div>}
        {!err && !md && <div className="ar-loading">loading {a.file}…</div>}
        {md && <article className="ar-prose md-prose" dangerouslySetInnerHTML={{__html: html}} />}
      </div>
    </div>
  );
}

function StreamItem({ m, t, onOpenArtifact }) {
  if (m.kind === 'system') {
    return (
  ...
  if (m.kind === 'doc-card') {
    return <DocCard artifactId={m.artifactId} onOpen={onOpenArtifact}/>;
  }
  if (m.kind === 'gate-card') {
    return (
      <div className="ch-msg allen">
